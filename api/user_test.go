...
//  use a customer matcher here because the same password will be hashed into different hashed password each time.
...
func randomUser(t *testing.T) (user db.User, password string) {
	...
	hashedPassword, err := util.CreateHashedPassword(password)
	require.NoError(t, err)
	user = db.User{
...
func requireBodyMatchUser(t *testing.T, body *bytes.Buffer, user db.User) {
	...
	require.NoError(t, err)
	var gotUser db.User
	err = json.Unmarshal(res, &gotUser)
	require.NoError(t, err)
